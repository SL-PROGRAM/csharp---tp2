...
namespace TP2
{
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            Console.WriteLine(ListeLivres.Max(x => x.Auteur.Nom)); ;
            ...
            foreach (var item in ListeAuteurs)
            {
                int pageMoyen = 0;
                int nbLivre = 0;
                foreach (var livre in ListeLivres.Where(x => x.Auteur == item))
                {
                    nbLivre++;
                    pageMoyen += livre.NbPages / nbLivre;

                }
                Console.WriteLine("{0} a écrit en moyenne {1} pages\n", item.Nom, pageMoyen);
            }

            Console.WriteLine("----------------------\n");
            Console.WriteLine("Afficher le titre du livre avec le plus de pages \n");
            Console.WriteLine(ListeLivres.Where(x => x.NbPages == ListeLivres.Max(y => y.NbPages)).First().Titre);


            Console.WriteLine("----------------------\n");
            Console.WriteLine("    Afficher combien ont gagné les auteurs en moyenne (moyenne des factures)  \n");
            foreach(var auteur in ListeAuteurs.Where(x => x.Factures.Count > 0))
            {
                Console.WriteLine(auteur.Nom + " :" + auteur.Factures.Sum(x => x.Montant)/auteur.Factures.Count);
            }
            ...
            Console.WriteLine("----------------------\n");
            Console.WriteLine("    Afficher les auteurs et la liste de leurs livres s \n");
            foreach (var auteur in ListeAuteurs)
            {
                Console.WriteLine(auteur.Nom);
                foreach (var livre in ListeLivres.Where(x => x.Auteur == auteur))
                {
            ...
            foreach ( var livre in ListeLivres.OrderBy(x => x.Titre))
            {
                Console.WriteLine("\t" + livre.Titre);
            }
            ...
            foreach(var livre  in ListeLivres.Where(
                x => x.NbPages > 
                (ListeLivres.Sum(y => y.NbPages)/ListeLivres.Count) ))
                {
                Console.WriteLine("\t" + livre.Titre);
            }
